...
class timthumb
{
	...
	static 	protected $curlDataWritten = 0;
	static 	protected $curlFH = false;
		...
		if (file_exists($this->cachefile)) {
			...
			if ($this->isURL) {
				...
				if (filesize($this->cachefile) < 1) {
					...
					if (WAIT_BETWEEN_FETCH_ERRORS < (time() - @filemtime($this->cachefile))) {
	...
	protected function processImageAndWriteToCache($localImage)
	{
		...
		if (((IMAGETYPE_PNG == $origType) || (IMAGETYPE_GIF == $origType)) && function_exists("imageistruecolor") && !imageistruecolor($image) && (0 < imagecolortransparent($image))) {
		...
		if (($imgType == "png") && OPTIPNG_ENABLED && OPTIPNG_PATH && @is_file(OPTIPNG_PATH)) {
			...
			$out = shell_exec("$exec -o1 $tempfile");
			clearstatcache();
	...
	protected function calcDocRoot()
	{
		...
		if (!isset($docRoot)) {
			...
			if (isset($_SERVER["SCRIPT_FILENAME"])) {
				$docRoot = str_replace("\\", "/", substr($_SERVER["SCRIPT_FILENAME"], 0, -strlen($_SERVER["PHP_SELF"])));
		...
		if (!isset($docRoot)) {
			...
			if (isset($_SERVER["PATH_TRANSLATED"])) {
				$docRoot = str_replace("\\", "/", substr(str_replace("\\\\", "\\", $_SERVER["PATH_TRANSLATED"]), 0, -strlen($_SERVER["PHP_SELF"])));
	...
	protected function serveWebshot()
	{
		...
		if (WEBSHOT_XVFB_RUNNING) {
			...
			$command = "$xv --server-args=\"-screen 0, $screenXx$screenYx$colDepth\" $cuty $proxy --max-wait=$timeout --user-agent=\"$ua\" --javascript=$jsOn --java=$javaOn --plugins=$pluginsOn --js-can-open-windows=off --url=\"$url\" --out-format=$format --out=$tempfile";
		}
	...
	static public function curlWrite($h, $d)
	{
		fwrite(self::$curlFH, $d);
		self::$curlDataWritten += strlen($d);

		if (MAX_FILE_SIZE < self::$curlDataWritten) {
			return 0;
		}
		else {
			return strlen($d);
	...
	protected function debug($level, $msg)
	{
		if (DEBUG_ON && ($level <= DEBUG_LEVEL)) {
			...
			if (0 < $this->lastBenchTime) {
				$tick = sprintf("%.6f", microtime(true) - $this->lastBenchTime);
			}

			$this->lastBenchTime = microtime(true);
			error_log("TimThumb Debug line " . 1131 . " [$execTime : $tick]: $msg");
		}
	}
	...
	static protected function returnBytes($size_str)
	{

...
$comment_author_avatar = ( isset($_POST['avatar']) )   ? trim($_POST['avatar']) : null;
...
if ( get_option('require_name_email') && !$user->exists() ) {
	...
	if ( '' == $comment_author )
		err( __('Error: please fill the required fields (name).') ); // 將 wp_die 改為錯誤提示
...
if ( $edit_id ){
	...
	update_comment_meta($comment_id, 'avatar', $comment_author_avatar);
} else {
	$comment_id = wp_new_comment( $commentdata );
	add_comment_meta($comment_id, 'avatar', $comment_author_avatar);
}
...
$total_comment = get_comments_number( $comment_post_ID );
if ($comment_author_avatar) $avatar = '<img class="avatar avatar-20 photo" width="20" height="20" src="'.$comment_author_avatar.'" alt="">';
else $avatar = get_avatar( $comment_author_email, '20' );
//以下是評論式樣, 不含 "回覆". 要用你模板的式樣 copy 覆蓋.
?>
<li class='comment_bar new_comment clearfix' id='comment-<?php echo $comment_id; ?>'>
	<div class='floor'><a href="<?php echo htmlspecialchars( get_comment_link( $comment_id ) ) ?>" target="_blank"><?php echo $total_comment;?> 楼</a></div>
	<div class='avatar_wrap'><?php echo $avatar; ?></div>
	<div class='comment_author'><a href='<?php echo $comment_author_url; ?>' rel='nofollow external' target='_blank'><?php echo $comment_author; ?></a></div>
	<?php if ( $comment->comment_approved == '0' ) : ?>
		<div class='moderation'>你的评论正在等待审核 ...</div>
	<?php else: ?>
